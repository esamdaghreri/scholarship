...
@section('content')
    <div class="wrapper flex flex-column just-cont-flex-start al-items-center">
        ...
        <div class="operation-scholarship-buttons flex">
            @if($extend_scholarship_on_progress_count == 0 && $cancel_scholarship_on_progress_count == 0 && $cancel_scholarship_success_count == 0 && $change_supervisor_scholarship_on_progress_count == 0 && $change_fellowship_scholarship_on_progress_count == 0 && $order->status->id == 1)
                ...
                <a href="{{route('changeFellowship.create', $order->id)}}" class="btn btn-primary">@lang('public.change_fellowship')</a>
            @endif
        </div>
    </div>
@endsection